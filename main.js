...
(function() {
    ...
    function getMockSearchResults(query, isAI) {
        const products = [
            ...
            {
                id: 7,
                name: 'کفش پیاده‌روی اسیکس جل-کایانو',
                brand: 'اسیکس',
                price: '3,950,000',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('load', function() {
      ...
      }, 3000); // تاخیر مناسب برای نمایش کامل لودر (قابل تنظیم)
    });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // ============ تنظیمات اصلی ============
    const settings = {
        slidesToShow: 5, // تعداد اسلایدهای قابل مشاهده در صفحه دسکتاپ
        autoplay: true, // پخش خودکار اسلایدر
        autoplaySpeed: 5000, // سرعت پخش خودکار (میلی‌ثانیه)
        rtl: true, // جهت راست به چپ
        transitionSpeed: 500, // سرعت انیمیشن (میلی‌ثانیه)
        breakpoints: {
            1200: { slidesToShow: 4 },
            992: { slidesToShow: 3 },
            768: { slidesToShow: 2 },
    ...
    // ============ انتخاب المان‌ها ============
    const sliderTrack = document.querySelector('.rail-slider-track');
    const slides = document.querySelectorAll('.rail-product-card');
    const prevButton = document.querySelector('.rail-nav-btn.rail-prev');
    const nextButton = document.querySelector('.rail-nav-btn.rail-next');
    const dots = document.querySelectorAll('.rail-dot');
    const addToCartButtons = document.querySelectorAll('.rail-add-to-cart');
    const wishlistButtons = document.querySelectorAll('.rail-add-wishlist');
    const quickViewButtons = document.querySelectorAll('.rail-quick-view');

    // بررسی وجود المان‌های ضروری
    if (!sliderTrack || slides.length === 0) {
        console.error("خطا: المان‌های اسلایدر یافت نشدند");
        return;
    }
    ...
    function initSlider() {
        ...
        // اطمینان از وجود اسلایدها
        if (slideCount === 0) return;
        
        // محاسبه تعداد اسلایدها بر اساس عرض صفحه
        updateSlidesPerView();
        
        // اضافه کردن data-product-id به کارت‌های محصول و دکمه‌ها
        slides.forEach((slide, index) => {
            const productId = slide.dataset.productId || `product${index + 1}`;
            slide.dataset.productId = productId;
            
            const addToCartBtn = slide.querySelector('.rail-add-to-cart');
            if (addToCartBtn) addToCartBtn.dataset.productId = productId;
            
            const wishlistBtn = slide.querySelector('.rail-add-wishlist');
            if (wishlistBtn) wishlistBtn.dataset.productId = productId;
            
            const quickViewBtn = slide.querySelector('.rail-quick-view');
            if (quickViewBtn) quickViewBtn.dataset.productId = productId;
        });
        
        // اضافه کردن قیمت‌ها به data-price
        addPricesToProducts();
        
        // بررسی وضعیت دکمه‌ها
        updateNavigationState();
        
        // تنظیم اسلایدر در موقعیت اول
        goToSlide(0, false);
    ...
    // بروزرسانی تعداد اسلایدهای قابل نمایش بر اساس عرض صفحه
    function updateSlidesPerView() {
        const windowWidth = window.innerWidth;
        
        // تنظیم تعداد اسلایدها بر اساس breakpoint
        slidesToShow = settings.slidesToShow; // مقدار پیش‌فرض
        
        // بررسی breakpoint‌ها از بزرگ به کوچک
        const breakpointsArray = Object.entries(settings.breakpoints).sort((a, b) => b[0] - a[0]);
        
        for (const [breakpoint, options] of breakpointsArray) {
    ...
    function goToSlide(index, animate = true) {
        ...
        // محدود کردن اندیس اسلاید به محدوده مجاز
        const maxIndex = Math.max(0, slideCount - slidesToShow);
        currentSlide = Math.min(Math.max(0, index), maxIndex);
        
        // محاسبه میزان جابجایی
        const translateValue = settings.rtl ? currentSlide * 100 / slidesToShow : -currentSlide * 100 / slidesToShow;
        
        if (animate) {
            isAnimating = true;
            sliderTrack.style.transition = `transform ${settings.transitionSpeed / 1000}s ease`;
    ...
    // بروزرسانی وضعیت دکمه‌های ناوبری
    function updateNavigationState() {
        if (!prevButton || !nextButton) return;
        
        const maxIndex = Math.max(0, slideCount - slidesToShow);
        
        // غیرفعال کردن دکمه قبلی در اسلاید اول
        prevButton.disabled = currentSlide === 0;
        prevButton.classList.toggle('disabled', currentSlide === 0);
        
        // غیرفعال کردن دکمه بعدی در اسلاید آخر
        nextButton.disabled = currentSlide >= maxIndex;
        nextButton.classList.toggle('disabled', currentSlide >= maxIndex);
    }
    ...
    function updateDots() {
        ...
        // محاسبه شماره صفحه فعلی (هر صفحه شامل چند اسلاید است)
        const totalPages = Math.ceil(slideCount / slidesToShow);
        const activePage = Math.floor(currentSlide / slidesToShow);
    ...
    function addPricesToProducts() {
        ...
        slides.forEach(slide => {
            ...
            const priceElement = slide.querySelector('.rail-price-current');
            
            // تلاش برای استخراج قیمت از متن
            let price = 0;
            if (priceElement) {
                price = extractPrice(priceElement.textContent);
            }
            
            // اگر قیمت استخراج نشد، از مقادیر پیش‌فرض استفاده کنیم
            if (price === 0 && productId && defaultPrices[productId]) {
                price = defaultPrices[productId];
            }
            
            // ذخیره قیمت در data-attribute
            slide.dataset.price = price;
            if (priceElement) priceElement.dataset.price = price;
        });
    }
    ...
    function addToCart(productId) {
        ...
        const productCard = document.querySelector(`.rail-product-card[data-product-id="${productId}"]`);
        if (!productCard) return false;
        
        const title = productCard.querySelector('.rail-product-title')?.textContent || 'محصول';
        const price = parseInt(productCard.dataset.price) || 0;
        const image = productCard.querySelector('.rail-product-image img')?.src || '';
        ...
        if (existingItemIndex >= 0) {
            ...
            showNotification(`تعداد ${title} در سبد خرید به ${cart[existingItemIndex].quantity} عدد افزایش یافت`);
        } else {
            // افزودن محصول جدید
            cart.push({
                id: productId,
            ...
            showNotification(`${title} به سبد خرید اضافه شد`);
        }
        
        // ذخیره و بروزرسانی UI
        saveCart();
        updateCartUI();
    ...
    function removeFromCart(productId) {
        ...
        if (cart.length !== initialLength) {
            ...
            // بازنشانی وضعیت دکمه محصول
            const addToCartBtn = document.querySelector(`.rail-add-to-cart[data-product-id="${productId}"]`);
            if (addToCartBtn) {
                addToCartBtn.classList.remove('in-cart');
                addToCartBtn.innerHTML = '<i class="ri-shopping-cart-line"></i> <span>افزودن به سبد</span>';
            }
    ...
    function updateCartUI() {
        // بروزرسانی تعداد آیتم‌ها در بج
        updateCartBadge();
        
        // بروزرسانی وضعیت دکمه‌های افزودن به سبد
        updateAddToCartButtons();
    }
    
    // بروزرسانی تعداد آیتم‌ها در بج سبد خرید
    function updateCartBadge() {
    ...
    // بروزرسانی وضعیت دکمه‌های افزودن به سبد خرید
    function updateAddToCartButtons() {
        addToCartButtons.forEach(button => {
            ...
            if (inCart) {
                button.innerHTML = '<i class="ri-check-line"></i> <span>در سبد خرید</span>';
            } else {
                button.innerHTML = '<i class="ri-shopping-cart-line"></i> <span>افزودن به سبد</span>';
            }
        });
    }
            ...
            cartItemElement.innerHTML = `
                ...
                <div class="cart-item-content">
                    ...
                    <div class="cart-item-bottom">
                        ...
                        <div class="cart-item-total">${formatPrice(itemTotalPrice)}</div>
                    </div>
                </div>
            `;
        ...
        notification.innerHTML = `
            <div class="notification-content">
                <div class="notification-icon">
                    <i class="${type === 'success' ? 'ri-check-line' : 'ri-information-line'}"></i>
                </div>
    ...
    function addNavigationEvents() {
        ...
        dots.forEach((dot, index) => {
            dot.addEventListener('click', () => {
                // محاسبه شماره اسلاید بر اساس شماره صفحه
                const slidesPerPage = slidesToShow;
                const targetSlide = index * slidesPerPage;
        ...
        addToCartButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                if (added) {
                    ...
                    this.innerHTML = '<i class="ri-check-line"></i> <span>در سبد خرید</span>';
                }
            });
        });
    }
    ...
    function init() {
        // بررسی وجود اسلایدر
        if (!sliderTrack || slides.length === 0) {
            console.error("اسلایدر پیدا نشد یا هیچ اسلایدی ندارد");
            return;
        }
        ...
        console.log("اسلایدر با موفقیت راه‌اندازی شد");
    }
    
    // فراخوانی تابع راه‌اندازی
    init();
});
document.addEventListener('DOMContentLoaded', function() {
    console.log("🛠️ اسکریپت رفع مشکل سبد خرید فعال شد");
    
    // دسترسی به توابع اصلی از main.js
    const mainOpenCartDrawer = window.openCartDrawer || function() {
        const cartDrawer = document.getElementById('cart-drawer');
        if (cartDrawer) {
            cartDrawer.classList.add('open');
            document.getElementById('modal-backdrop')?.classList.add('active');
    ...
    // اصلاح تابع نمایش سبد خرید
    function fixRenderCart() {
        console.log("📦 شروع اصلاح تابع نمایش سبد خرید");
        
        // بررسی سبد خرید در localStorage
        let cart = [];
        try {
            const savedCart = localStorage.getItem('cartItems');
            if (savedCart) {
                ...
                console.log("⚠️ سبد خرید خالی است یا ذخیره نشده است");
            }
        } catch (e) {
    ...
    // تابع نمایش آیتم‌های سبد خرید
    function renderCartItems(cart) {
        const cartItemsContainer = document.querySelector('.cart-items');
        if (!cartItemsContainer) {
            console.error("❌ المان cart-items پیدا نشد!");
            return;
        }
        
        console.log("🔄 شروع رندر کردن آیتم‌های سبد خرید");
        ...
        if (!cart || cart.length === 0) {
            ...
            console.log("ℹ️ سبد خرید خالی نمایش داده شد");
            return;
        }
        ...
        console.log(`✅ ${cart.length} آیتم در سبد خرید نمایش داده شد`);
    ...
    function updateCartSummary(totalPrice) {
        ...
        if (summaryElements.length >= 4) {
            ...
            // مجموع قیمت
            summaryElements[0].querySelector('span:nth-child(2)').textContent = formattedTotal;
            
            // مبلغ قابل پرداخت
            summaryElements[3].querySelector('span:nth-child(2)').textContent = formattedTotal;
            
            console.log(`💰 جمع کل سبد خرید بروزرسانی شد: ${formattedTotal}`);
        }
    ...
    function updateCartBadge(cart) {
        ...
        if (badge) {
            badge.textContent = totalItems;
            badge.style.display = totalItems > 0 ? 'flex' : 'none';
            console.log(`🔢 بج سبد خرید بروزرسانی شد: ${totalItems}`);
        }
    }
    ...
    function addCartItemEvents() {
        ...
        console.log("🔗 رویدادهای سبد خرید اضافه شدند");
    }
        ...
        try {
            ...
            if (savedCart) {
                ...
                if (cart[index]) {
                    ...
                    console.log(`🔄 تعداد محصول با شاخص ${index} تغییر کرد: ${change > 0 ? '+' : ''}${change}`);
                }
            }
        } catch (e) {
            console.error("❌ خطا در بروزرسانی تعداد محصول:", e);
        }
    }
        ...
        try {
            ...
            if (savedCart) {
                ...
                if (index >= 0 && index < cart.length) {
                    ...
                    console.log(`🗑️ محصول از سبد خرید حذف شد:`, removedItem);
    ...
    // بازنویسی تابع باز کردن سبد خرید
    window.openCartDrawer = function() {
        mainOpenCartDrawer(); // فراخوانی تابع اصلی
        
        // بارگذاری مجدد محتویات سبد خرید
        setTimeout(fixRenderCart, 50);
        
        console.log("🛒 سبد خرید باز شد و محتویات بروزرسانی شدند");
    };
    
    // رویدادهای سبد خرید
    function attachCartEvents() {
        // اجرای فیکس هنگام باز کردن سبد خرید
        document.querySelector('.cart-toggle')?.addEventListener('click', function() {
            setTimeout(fixRenderCart, 100);
        });
        
        // دکمه‌های افزودن به سبد خرید
        document.querySelectorAll('.rail-add-to-cart, .add-to-cart-btn').forEach(button => {
            button.addEventListener('click', function() {
                setTimeout(function() {
                    // شبیه‌سازی باز کردن سبد خرید برای تست
    ...
    // شروع اصلاحات
    attachCartEvents();
    console.log("✅ فیکس سبد خرید با موفقیت نصب شد");
});
